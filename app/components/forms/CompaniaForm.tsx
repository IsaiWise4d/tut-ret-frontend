...
import { createCompaniaSeguros, updateCompaniaSeguros } from '@/app/lib/api';

interface CompaniaFormProps {
    initialData?: CompaniaSeguros;
    onSuccess: () => void;
    onCancel: () => void;
}

export default function CompaniaForm({ initialData, onSuccess, onCancel }: CompaniaFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && initialData) {
                await updateCompaniaSeguros(initialData.id, { nombre, direccion });
            } else {
                await createCompaniaSeguros({ nombre, direccion });
            }
            onSuccess();
        } catch (err: any) {

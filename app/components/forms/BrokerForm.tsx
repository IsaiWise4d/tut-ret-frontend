...
import { createBroker, updateBroker } from '@/app/lib/api';
...
export default function BrokerForm({ initialData, onSuccess, onCancel }: BrokerFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isEditing && initialData) {
                await updateBroker(initialData.id, { nombre, direccion });
            } else {
                await createBroker({ nombre, direccion });
            }
            onSuccess();
        } catch (err: any) {
            setError(err.message || 'Error al guardar el broker');
        } finally {
            setLoading(false);
        }
    };
